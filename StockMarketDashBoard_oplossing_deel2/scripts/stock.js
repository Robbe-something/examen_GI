...
export let stockTesla = new Observable(subscriber => {
    let stockTesla = new StockSignal("data/tesla.json");
    setInterval(() => subscriber.next([stockTesla.date, stockTesla.value]), 1000);
});

export let stockMeta = new Observable(subscriber => {
    let stockMeta = new StockSignal("data/meta.json");
    setInterval(() => subscriber.next([stockMeta.date, stockMeta.value]), 1000);
});


let stockAlphabetHotSignal = new StockSignal("data/alphabet.json");
export let stockAlphabetHot = new Observable(subscriber => {
    setInterval(() => subscriber.next([stockAlphabetHotSignal.date, stockAlphabetHotSignal.value]), 1000);
});
